...
namespace Controllers.UserAuth
{
    public class UserSignInController : ISignInRequestedSubscriber, IDisposable
    {
        ...
        private async Task SignIn(string username, string password)
        {
            ...
            if (result == null)
            {
                ...
                EventBus.EventBus.RaiseEvent<IAuthSuccessfullySubscriber>(async sub =>
                    await sub.HandleAuthSuccess(new AuthResult { Success = false, Message = "Невозможно войти" }));
                return;
            }
        ...
        private static async Task ProcessToken(JwtTokensData tokensData)
        {
            ...
            EventBus.EventBus.RaiseEvent<IAuthSuccessfullySubscriber>(async sub => await sub.HandleAuthSuccess(new AuthResult()
            {
                Success = true
            }));
